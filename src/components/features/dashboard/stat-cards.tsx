import { Calendar, Users, CheckSquare, Wallet, TrendingUp, TrendingDown } from "lucide-react"
...
export function StatCards({ filter = "7days", eventTypeFilter = "all", customRange }: StatCardsProps) {
  const { data, isLoading, error } = useDashboardStats(filter, eventTypeFilter, customRange)
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
      {stats.map((stat) => (
        ...
        >
          ...
          {stat.isCurrency && data.budget.total > 0 && (
            <div className="mt-2">
              <div className="h-1.5 bg-[#f3f4f6] rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-[#EF4444] to-[#F97316] rounded-full"
                  style={{
                    width: `${(stat.data.breakdown[0].value / data.budget.total) * 100}%`,
                  }}
                />
              </div>
              <p className="text-[10px] text-[#6b7280] mt-0.5">
                {Math.round((stat.data.breakdown[0].value / data.budget.total) * 100)}% utilisé
              </p>
            </div>
          )}
        </div>
